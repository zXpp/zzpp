...
#TNet=$NNROOT/bin/TNet${CUDA:+Cu}	# change made by Yulan Liu, 10 Sep 2013
TNet=./TNet
echo
echo This script: $0
echo
...
echo MAX_ITER: ${MAX_ITER:=100}
echo MIN_ITER: ${MIN_ITER:=30}
...
nnet_base=$NNMODEL/$(basename $NN_INIT .init)
...
for iter in $(seq -w $MAX_ITER); do
  ...
  run_tnet_parse_accu "$cmd"
  accu_train=$ACCU
  echo "TR accuracy:  $ACCU iter: $iter learnrate: $LEARNRATE"\
   ${MOMENTUM:+" momentum: $MOMENTUM"}\
   ${WEIGHTCOST:+" weightcost: $WEIGHTCOST"}
  ...
  run_tnet_parse_accu "$cmd"
  accu_cv=$ACCU
  echo "CV accuracy: $ACCU iter: $iter learnrate: $LEARNRATE"\
   ${MOMENTUM:+" momentum: $MOMENTUM"}\
   ${WEIGHTCOST:+" weightcost: $WEIGHTCOST"}

  nnet_next_accu=${nnet_next}_lr$(printf '%.5g' $LEARNRATE)_tr$(printf '%.5g' $accu_train)_cv$(printf '%.5g' $accu_cv)
  mv $nnet_next $nnet_next_accu

  # always accept the weights when fixed LEARNRATE by keep_LEARNRATE_iter
  if [[ ${iter##0} -lt $KEEP_LRATE_ITER ]]; then
    echo 'accepting weights (keep_LEARNRATE_iter)'
