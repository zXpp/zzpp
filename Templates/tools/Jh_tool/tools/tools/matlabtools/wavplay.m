...
%   $Revision: 1.1.6.7 $  $Date: 2009/04/15 23:17:19 $

if ~ispc
   error('MATLAB:wavplay:invalidPlatform','WAVPLAY is only for use with Windows machines.');
end
...
if ischar(varargin{end}),
   ...
   if nargin>2,
      error('MATLAB:wavplay:invalidInputs','Invalid input arguments.');
   end
   s='sync';
end
...
if ndims(y)>2, error('MATLAB:wavplay:invalidInputType','Input must be a 2-D matrix.'); end

sync_opt = strmatch(s,{'sync','async'});
if isempty(sync_opt),
   error('MATLAB:wavplay:unrecognizedOption','Unrecognized option: %s',s);
end
...
if sync_opt==2,
   ...
   case 'uint8'
      ...
      bits = 8;
   otherwise,
      error('MATLAB:wavplay:invalidDataType','Data must be of type ''double'', ''single'', ''int16'', or ''uint8''.');
   end
