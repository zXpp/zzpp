...
def score(f, ref, uem, sys):
    der_score = "/share/spandh.ami1/usr/rosanna/tools/md-eval.pl -as -c 0.25 -r %s %s -s %s > %s.output.der" % (ref, uem, sys, sys)
...
def findboundary(ref, sys, stat):
    ...
    output = commands.getoutput("/share/spandh.ami1/sw/spl/octave/v3.6.3/x86_64/bin/octave '%s' > %s.output" % (mscript, sys) )
    overall_score = 0
    nmatches = 0
    nins = 0
    ndel = 0
...
def findtotal(stats, num_files):
	...
	asp += stat[7][1]
	k += stat[7][2]
    return ["'''AVG'''", miss_per/num_files, falarm_per/num_files, spkrerr_per/num_files, der_per/num_files, overall_score/num_files, nmatches_per/num_files, nins_per/num_files, ndel_per/num_files, err_per/num_files, acp/num_files, asp/num_files, k/num_files]

# ------------------------------------------------
def printstats(stats, total, mode):
	...
	print header
		...
		print line
    	if len(stats) > 1:
		line = "|| %s || %.2f  || %.2f  || %.2f  || %.2f || || || || || ||" % (total[0].split("/")[len(total[0].split("/"))-1], total[1], total[2], total[3], total[4])
		if "-dp" in mode:
			line += " || %.2f || %.2f || %.2f || %.2f || %.2f ||" % (total[5], total[6], total[7], total[8], total[9])
		if "-pu" in mode:
			line += " || %.2f || %.2f || %.2f ||" % (total[10], total[11], total[12])
		print line
...
if num_files == 0:
	num_files = 1
total = findtotal(stats, num_files)
printstats(stats, total, mode)
