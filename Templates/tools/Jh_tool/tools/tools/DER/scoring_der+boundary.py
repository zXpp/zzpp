...
#rosanna
# USAGE: python script.py -rl /path/to/referencelist/file -sp /path/to/sysrttm/folder/ (--speech)
# ------------------------------------------------
...
def score(f, ref, uem, sys):
    der_score = "/share/spandh.ami1/usr/yanxiong/tools/DER/md-eval.pl -as -c 0.25 -r %s -u %s -s %s > output.der" % (ref, uem, sys)
# "import commands" must be written first before using commands.getoutput 
    commands.getoutput(der_score) # use getoutput() to receive only the text output from the command.
    miss = 0.0
    miss_per = 0.0
    falarm = 0.0
    falarm_per = 0.0
...
def findboundary(ref, sys, stat):
    ...
    output = commands.getoutput("/share/spandh.ami1/sw/spl/octave/v3.6.3/x86_64/bin/octave '/share/spandh.ami1/usr/yanxiong/tools/DER/get_scores.m' > output")
    overall_score = 0
    nmatches = 0
    nins = 0
    ndel = 0
...
for f in files:
    print f
    if speech == "on":
        ref = "/share/spandh.ami1/usr/yanxiong/data/tbl/speech.ref.rttm/%s.speech.ref.rttm" % f
        print '\n speech.ref.rttm is here: '
    else:
    	ref = "/share/spandh.ami1/usr/yanxiong/data/tbl/ref.rttm/%s.ref.rttm" % f
        print '\n ref.rttm is here: '
    print ref

    uem = "/share/spandh.ami1/usr/yanxiong/data/tbl/uem/%s.uem" % f
    print '\n uem is here: '
    print uem 
    sys = syspath + "/" + f + ".sys.rttm"
    print '\nsystem output files are: '
    print sys
