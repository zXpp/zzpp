...
sub ConfusionMatrix
{
    ...
    printf("Segment CONFUSION MATRIX\n");
    printf("%-15s   ","TEST --->");
    foreach $t ( @TKEYS ){
        printf("%8s ",$t);
    }
    ...
    foreach $r ( sort keys %CONF ) {
        printf("%-15s : ",$r);
        foreach $t ( @TKEYS ){
            printf($FMTPERC_D . " ",  $CONF{$r}{$t});
           # printf($FMTPERC . " ", $SegNum );
            $totalCF += 100 * $CONF{$r}{$t} / $TotalNum;  ######## SA:Check total of the whole confusion matrix
        }
        print "\n";
    }
    print "-" x (@TKEYS * 9 + 18) . "\n";
    printf ("Total of Segment confusion matrix : " . $FMTPERC . "\n",  $totalCF ); ######## SA:Check total of the whole confusion matrix
    ...
    foreach $r ( sort keys %CONF ) {
        printf("%-15s : ",$r);
        foreach $t ( @TKEYS ){
            printf($FMTPERC . " ", 100 * $CONF{$r}{$t} / $SegNum{$r} );
        }
        print "\n";
    }
    print "-" x (@TKEYS * 9 + 18) . "\n";
