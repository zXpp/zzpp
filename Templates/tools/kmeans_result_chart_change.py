...
njname=clures+'.k.res' 
chart_file=clures+'.xls'#630_21.k.res
f3=open(njname,'wa')
...
for i in range(0,modelnum): #21 spkers0-20
	...
	col_col+=[str(i+1)]
	ind_col+=[str(i)]
chart=DataFrame(data,index=col_col,columns=ind_col)
chart.columns=brandnamelist
chart.index=brandnamelist
