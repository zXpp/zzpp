...
with open(path+'wld.efn_'+dim_commend+'.xml','wa',0) as fwrite:
    ...
    iter=[]
    iter_total_t=[]
    iter_total_cov=total[84:(84+2500)]
    iter_total_mean=total[22:(22+50)]
    for iter_index in range(5):
        begin=(2597+5105*iter_index)
        end=2597+5105*(iter_index+1)
        iter.insert(iter_index,total[begin:end])
        iter_tmp_all=iter[iter_index]
        if iter_index !=0:
            iter_total_mean+=iter_tmp_all[22:22+50]
            iter_total_cov+=iter_tmp_all[84:(84+2500)]  ## all data of 5 iters  cov 2500*5=12500
        iter_total_t+=iter_tmp_all[2601:(2601+2500)] ## all data of 5 iters t 2500*5=12500
    ...
    fwrite.writelines(ele_golbal_mean for ele_golbal_mean in sample(iter_total_mean,dim))
    ...
    fwrite.writelines(golbal_cov_Data_1 for golbal_cov_Data_1 in sample(iter_total_cov,dim*real_dim))
...
#==============================================================================
    ...
    for i in range(5):
        ...
        if i !=0:
            fwrite.writelines(ele_iter_head_mean for ele_iter_head_mean in sample(iter_total_mean,dim))
        else:
    ...
    #        col.insert(i,covariance[50*i:50*(i+1)])
            if i!=0:
                ...
                fwrite.writelines(iter_cov_Data for iter_cov_Data in sample(covariance_iter,dim))
    #    fwrite.writelines(iter_cov_Data_51 for iter_cov_Data_51 in covariance_iter[50:100])
        if i!=0:
            ...
            fwrite.writelines(iter_cov_Data_1 for iter_cov_Data_1 in sample(covariance_iter,dim*real_dim))
    #    for p in range(dim):
