...
#     if label.decode('gbk') in single_col:#单行{一级规格、耳机规格}
    for ele in enumerate(single_col) :#col_cont为表格中该列下区域
        if ele and ele[1] not in ['',label.decode('gbk')]:
            contain.append(ele)
    if contain:
        ...
        for i in range(contain[0][0]+1,len(single_col)):
            if single_col[i] == unicode(u''):
                if single_col[i-1] == unicode(u''):
                ...
                for col_ind in range(0,ncols):
                    ...
                    if ('规格编号' or '描述' or '序号' or '版本支持情况' or '支持属性') not in col_cont[:20] :
                        ...
                        if '一级规格' in col_cont:
                            ...
                            dev_name=col_cont[col_cont.index(unicode(u'支持属性'))-2] if col_cont[col_cont.index(unicode(u'支持属性'))-2] != '' else col_cont[col_cont.index(unicode(u'支持属性'))-3] #2,3,4
                            ...
                            if unicode(u'规格说明') in col_cont:
                                ...
                                for x in range(len(col_cont)):
                                    if col_cont[x] not in ['','规格说明']:
                                        beg_ind=x
                                        break
                                dev_val=col_cont[beg_ind:]  ##缁岃桨绨?,7l娑撱倛顢?
