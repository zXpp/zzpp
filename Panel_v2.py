...
#print(ppt_diago)
	...
	width = args["_newsize"]
	pyrealpath=os.path.split(sys.argv[0])[0]
	pdfout,tmp= 1,os.path.join(raw,label,str(width))
	tmp2 =os.path.join(raw,label,"Slides")
	newtitle=args["_outname"]
	if 2 not in args["_outFormat"]:pdfout=0
	##################CHQ RU JIANQIE VB CHENGXU
	cmd=" ".join([r"C:\Windows\system32\wscript.exe "+os.path.join(pyrealpath,r"util\Tmp\PPT_Panel.vbs"),vbpptname,str(width),vbout,str(pdfout)])
